...
class ActorCriticEncoder(nn.Module):
    ...
    def _encode_terrain(self, obs):
        ...
        mha_output, head_attention_weights = self.mha(
            query=proprio_embedding,
            key=local_features,
            value=local_features,
            average_attn_weights=False,
        )
        avg_attention_weights = head_attention_weights.mean(dim=1)
        ...
        return encoded_obs, avg_attention_weights, head_attention_weights
    ...
    def act_inference(self, obs):
        actor_obs = self.get_actor_obs(obs)
        actor_obs = self.actor_obs_normalizer(actor_obs)
        encoded_obs, avg_attention_weights, head_attention_weights = self._encode_terrain(actor_obs)
        return self.actor(encoded_obs), avg_attention_weights, head_attention_weights
